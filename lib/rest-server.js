...
/*
  ...
  router.post('/video/subscribe', function subscribe(req, res, next) {
    var params = _.extend({}, req.params, req.body);
    var streamId = +params.streamId;
    //self.janusClient.startVideo(null).then(function(){
  ...
  router.post('/video/start', function subscribe(req, res, next) {
    var params = _.extend({}, req.params, req.body);
    var jsep = params.jsep;
    self.janusClient.startVideo(JSON.parse(jsep)).then(function(result) {
      res.send(result);
      return next();
    });
  });
