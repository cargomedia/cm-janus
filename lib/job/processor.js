...
JobProcessor.prototype.processUntilSuccessful = function(job) {
  ...
  return this.process(job).catch(function(error) {
    serviceLocator.get('logger').warn('Job processing failed. Reattempt.', job.context.clone().extend({error: error}).toHash());
...
JobProcessor.prototype._createJobWorkingDirectory = function(job) {
  ...
  try {
    ...
    serviceLocator.get('logger').error('Job working directory could not be created', job.context.clone().extend({error: err}).toHash());
    throw err;
  }
  return dir;
};
