...
JobManager.prototype._processJobFile = function(filePath) {
  ...
  return self._readJobFile(filePath)
    .then(function(jobDescription) {
      ...
      context.merge(job.context);
      return self._processor.processUntilSuccessful(job);
    })
    .then(function() {
      serviceLocator.get('logger').info('Removing job file', context.clone().extend({filepath: filePath}).toHash());
      return fs.unlinkAsync(filePath);
    })
    .catch(function(error) {
      serviceLocator.get('logger').error('Processing job file failed', context.clone().extend({error: error}).toHash());
    });
};
