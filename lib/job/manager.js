...
var JobClassRegistry = require('./class-registry');
...
function JobManager(jobDirectory, tempFilesDir, jobWatchList) {
  ...
  this._watch = null;
  this._jobClassRegistry = null;
  this._activeJobs = {};
  this._setupJobClassRegistry(jobWatchList);
}
 ...
 * @param {Array} jobWatchList
 */
JobManager.prototype._setupJobClassRegistry = function(jobWatchList) {
  this._jobClassRegistry = new JobClassRegistry();
  jobWatchList.forEach(function(jobClass) {
    this._jobClassRegistry.register(jobClass);
  }.bind(this));
};
...
JobManager.prototype._createJob = function(plugin, event, data) {
  var jobClass = this._jobClassRegistry.getJobClass(plugin, event, data);
