 ...
 * @constructor
 */
function AbstractJob(id, jobData, configuration, context) {
 ...
 * @returns {Object}
 */
AbstractJob.prototype.getContext = function() {
...
AbstractJob.prototype._runJobScript = function(commandText) {
  ...
  serviceLocator.get('logger').debug('Starting job process', this.context.clone().extend({command: commandText}).toHash());
  var options = {
  ...
  return this._spawn(command, commandArgs, options)
    ...
    .catch(function(error) {
      serviceLocator.get('logger').warn('Failed job process', self.context.clone().extend({command: commandText}).toHash());
      throw error;
    })
    .finally(function() {
      self._process = null;
