...
AbstractJob.prototype.getContext = function() {
  return new Context({job: this.id});
};
...
AbstractJob.prototype._runJobScript = function(commandText) {
  ...
  serviceLocator.get('logger').debug('Starting job process', this.getContext().extend({command: commandText}));
  var options = {
  ...
  return this._spawn(command, commandArgs, options)
    ...
    .catch(function(error) {
      serviceLocator.get('logger').warn('Failed job process', self.getContext().extend({command: commandText}));
      throw error;
    })
    .finally(function() {
      self._process = null;
