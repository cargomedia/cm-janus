        ...
        .then(function() {
          return Promise.join(
            unlink(audioMjrFile).catch(function(error) {
              serviceLocator.get('logger').error('Removing audio file failed', self.context.clone().extend({error: error}).toHash());
            }),
            unlink(videoMjrFile).catch(function(error) {
              serviceLocator.get('logger').error('Removing video file failed', self.context.clone().extend({error: error}).toHash());
            })
          );
        });
    });
