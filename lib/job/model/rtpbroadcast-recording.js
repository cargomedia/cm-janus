        ...
        .then(function() {
          return Promise.join(
            unlink(audioMjrFile).catch(function(error) {
              serviceLocator.get('logger').error('Removing audio file failed', self.getContext().extend({error: error}));
            }),
            unlink(videoMjrFile).catch(function(error) {
              serviceLocator.get('logger').error('Removing video file failed', self.getContext().extend({error: error}));
            })
          );
        });
    });
