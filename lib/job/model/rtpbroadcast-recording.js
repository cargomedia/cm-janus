        ...
        .then(function() {
          return Promise.join(
            unlink(audioMjrFile).catch(function(error) {
              serviceLocator.get('logger').warn('Removing audio file failed', context.clone().extend({exception: error}));
            }),
            unlink(videoMjrFile).catch(function(error) {
              serviceLocator.get('logger').warn('Removing video file failed', context.clone().extend({exception: error}));
            })
          );
        });
    });
