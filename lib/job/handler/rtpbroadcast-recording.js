...
RtpbroadcastRecordingHandler.prototype._audioVideoMerge = function(audioFileName, videoFilename, mpegFileName) {
  var shellScript = path.join(__dirname, '/../../../bin/', 'rtpbroadcast-merge.sh');
  return new Promise(function(resolve, reject) {
    this._exec([shellScript, audioFileName, videoFilename, mpegFileName].join(' '), function(error) {
      if (null === error) {
        resolve();
      } else {
        reject(error);
