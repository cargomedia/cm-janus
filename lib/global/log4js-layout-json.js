  ...
  process: function(logEvent) {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        var params = this.extractParams(data);
        logEntry = _.extend(logEntry, params);
      }
  ...
  extractParams: function(logData) {
    ...
    logData.forEach(function(element) {
      if (isPlainObject(element)) {
        extra = _.extend(extra, element);
      } else {
        messages.push(element);
      }
    });
    var parsedMessages = this.parseMessages(messages);
    extra['message'] = parsedMessages.shift();
    if (parsedMessages.length > 0) {
      extra['additionalMessages'] = parsedMessages;
    }
    return extra;
  },
