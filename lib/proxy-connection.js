...
ProxyConnection.prototype.installListeners = function() {
  var self = this;
  this.browserConnection.on('message', function(request) {
    self.processMessage(request).then(function() {
      ...
      //error has been caught and logged by processMessage
    });
  });
  ...
  this.browserConnection.on('error', this._websocketError.bind(this));
  this.janusConnection.on('close', this.close.bind(this));
  this.janusConnection.on('error', this._websocketError.bind(this));
};
...
ProxyConnection.prototype.processMessage = function(message) {
  return this._processMessage(message).catch(function(error) {
    return this._processMessageError(error);
  }.bind(this));
};
 ...
 * @returns {Promise}
 */
ProxyConnection.prototype._processMessageError = function(error) {
  serviceLocator.get('logger').error('"' + this.identifier + '" processMessage error', error);
  return this._unexpectedError(error).then(function() {
    throw error;
  });
};
 ...
 * @param {Error} error
 */
ProxyConnection.prototype._websocketError = function(error) {
  serviceLocator.get('logger').error('"' + this.identifier + '" websocket error', error);
 ...
 * @returns {Promise}
 */
ProxyConnection.prototype._unexpectedError = function(error) {
  if (!(error instanceof JanusError.Error)) {
    error = new JanusError.Unknown(ProxyConnection.generateTransactionId());
  }
  var self = this;
  return this.browserConnection.send(error.response).finally(function() {
