 ...
 * @constructor
 */
function ProxyConnection(serviceLocator, browserConnection, janusConnection) {
...
ProxyConnection.prototype.processMessage = function(message) {
  ...
  return this._processMessage(message).catch(function(error) {
    if (!error instanceof JanusError.Error) {
      this.serviceLocator.get('logger').error('Unexpected error', error);
...
ProxyConnection.prototype.close = function() {
  ...
  streams.findAllByConnection(this).forEach(function(stream) {
    ...
    this.serviceLocator.get('logger').info('removing stream', stream);
    this.serviceLocator.get('cm-api-client').removeStream(stream.channelName, stream.id);
  }.bind(this));
  this.sessionId = null;
};
...
ProxyConnection.prototype.stopStream = function(streamId) {
  return new Promise(function(resolve) {
    var streams = this.serviceLocator.get('streams');
