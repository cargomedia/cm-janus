...
Connection.prototype._send = function(message) {
  ...
  if (!message.transaction) {
    ...
    promise = new Promise(function(resolve, reject) {
      ...
      this.pendingTransactions[transactionId] = {resolve: resolve, reject: reject};
    }.bind(this));
  }
  ...
  if (message.transaction) {
    ...
    if (pendingTransaction) {
      if ('success' === message.janus) {
        pendingTransaction.resolve(message);
        delete this.pendingTransactions[message.transaction];
      }
      if ('error' === message.janus) {
        pendingTransaction.reject(message);
        delete this.pendingTransactions[message.transaction];
      }
    }
  }
