...
function HttpServer(port, apiKey) {
  ...
  process.on('uncaughtException', function(err) {
    serviceLocator.get('logger').error('Uncaught exception ' + err.stack || err.message);
...
HttpServer.prototype.installRoutes = function(router) {
  ...
  router.get('/status', function subscribe(req, res, next) {
    var result = _.map(serviceLocator.get('streams').list, function(stream) {
      return {id: stream.id, channelName: stream.channelName, isPublish: stream.isPublish};
    });
    res.send(result);
    return next();
  });
...
HttpServer.prototype.start = function() {
  ...
  serviceLocator.get('logger').debug('HTTP server on port ' + this.port + ' started');
};
