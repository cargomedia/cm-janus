...
/**
 * @param {Transform} logger
 */
var HttpServer = function(logger) {
  this.logger = logger;
...
HttpServer.prototype.installRoutes = function(router) {
  ...
  if (!config.has('httpServer.apiKey')) {
    throw new Error('Secret apiKey isn\'t set');
  }
  var apiKey = config.get('httpServer.apiKey');

  router.use(function(req, res, next) {
    self.logger.debug('request ' + req.path);
...
HttpServer.prototype.start = function() {
  ...
  this.logger.debug('HTTP server on port ' + listenPort + ' started');
};
