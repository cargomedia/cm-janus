          ...
          stream.plugin.removeStream()
            .then(function() {
              if (stream.plugin.stream || serviceLocator.get('streams').has(streamId)) {
                throw new Error('Could not remove stream by force');
              } else {
                serviceLocator.get('logger').warn('Stream stop failed. Stream was removed by force', context);
                res.send({success: 'Stream stopped'});
                next();
              }
            })
            .catch(function(error) {
              var message = 'Stream stop failed. Stream was not removed.';
