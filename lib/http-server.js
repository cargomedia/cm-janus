 ...
 * @constructor
 */
function HttpServer(serviceLocator, port, apiKey) {
  if (!apiKey) {
  ...
  process.on('uncaughtException', function(err) {
    self.serviceLocator.get('logger').error('Uncaught exception ' + err.stack || err.message);
...
HttpServer.prototype.installRoutes = function(router) {
  ...
  router.use(function(req, res, next) {
    self.serviceLocator.get('logger').debug('request ' + req.path);
    ...
    if (stream) {
      stream.proxyConnection.stopStream(stream.id)
        .then(function() {
          ...
          self.serviceLocator.get('logger').warn('Stream stop failed', error);
  ...
  router.get('/status', function subscribe(req, res, next) {
    var result = _.map(serviceLocator.get('streams').list, function(stream) {
...
HttpServer.prototype.start = function() {
  ...
  this.serviceLocator.get('logger').debug('HTTP server on port ' + this.port + ' started');
};
