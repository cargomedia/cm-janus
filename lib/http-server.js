...
function HttpServer(port, apiKey) {
  ...
  app.use(function(err, req, res, next) {
    var context = new Context({error: err});
    ...
    if (stream) {
      ...
      serviceLocator.get('http-client').detach(stream.plugin)
        ...
        .catch(function(error) {
          serviceLocator.get('logger').warn('Stream stop failed', context.clone().extend({error: error}));
          serviceLocator.get('logger').info('Removing stream by force', context);
