...
function JanusProxy(listenPort, janusAddress) {
  ...
  this._connections = {};
  this._connectionCloseListeners = {};
}
...
JanusProxy.prototype.start = function() {
  ...
  webSocketServer.on('connection', function(incomingConnection) {
    try {
      ...
      if (proxyConnection) {
        proxy._stopConnection(proxyConnection);
      }
    }
  });
};
...
JanusProxy.prototype.stop = function() {
  _.each(this._connections, function(proxyConnection) {
...
JanusProxy.prototype._addConnection = function(proxyConnection) {
  ...
  this._connections[proxyConnection.id] = proxyConnection;
  this._connectionCloseListeners[proxyConnection.id] = closeListener;
...
JanusProxy.prototype._removeConnection = function(proxyConnection) {
  ...
  delete this._connectionCloseListeners[proxyConnection.id];
};
...
JanusProxy.prototype._stopConnection = function(proxyConnection) {
  var closeListener = this._connectionCloseListeners[proxyConnection.id];
