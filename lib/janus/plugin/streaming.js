...
PluginStreaming.prototype.subscribe = function(message) {
  var plugin = this;
  return serviceLocator.get('cm-api-client').subscribe(plugin.stream.channelName, plugin.stream.id, plugin.stream.start.getTime() / 1000, plugin.session.data, plugin.stream.channelData)
    .then(function() {
