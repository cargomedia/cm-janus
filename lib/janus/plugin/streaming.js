...
PluginStreaming.prototype.onCreate = function(message) {
  ...
  this.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      var channelData = message['body']['channel_data'];
      plugin.stream = Stream.generate(message['body']['id'], plugin, channelData);
      serviceLocator.get('logger').info('Added ' + plugin.stream + ' for streaming ' + plugin + ' with channelData ' + plugin.stream.channelData);

      return serviceLocator.get('cm-api-client').publish(plugin.stream.channelName, plugin.stream.id, Date.now() / 1000, plugin.session.data, channelData)
        .then(function() {
