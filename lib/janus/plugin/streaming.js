  ...
  return (new Promise(function(resolve, reject) {
    ...
    .catch(JanusError.Error, function(error) {
      serviceLocator.get('http-client').detach(plugin).catch(function(error) {
        serviceLocator.get('logger').warn('Failed detach of plugin', plugin.getContext().extend({exception: error}));
      });
  ...
  return (new Promise(function(resolve, reject) {
    plugin.queue.push(function() {
      ...
      if (!plugin.stream) {
        ...
        promise = serviceLocator.get('streams').unregister(plugin.stream).then(function() {
          plugin.stream = null;
        })
      }
