...
PluginStreaming.prototype.onCreate = function(message) {
  ...
  this.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      ...
      return serviceLocator.get('cm-api-client').publish(plugin.stream.channelName, plugin.stream.id, plugin.stream.start.getTime() / 1000, plugin.session.data, plugin.stream.channelData)
        .then(function() {
