 ...
 * @returns {Promise}
 */
PluginVideo.prototype.processRequest = function(message) {
  ...
  return PluginVideo.super_.prototype.processRequest.call(this, message);
};
...
PluginVideo.prototype.onCreate = function(message) {
  var self = this;
  this.JanusConnection.transactions.add(message['transaction'], function(response) {
    if (self._isSuccessResponse(response)) {
      self.stream = Stream.generate(message['body']['id'], self.JanusConnection);
      return serviceLocator.get('cm-api-client').publish(self.stream.channelName, self.stream.id, Date.now(), self.JanusConnection.sessionData)
        .then(function() {
