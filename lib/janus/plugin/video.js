...
PluginVideo.prototype.onCreate = function(message) {
  ...
  this.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      var channel = new Channel(response['plugindata']['data']['stream']['uid'], message['body']['id'], message['body']['channel_data'], plugin.context.clone());
      plugin.stream = Stream.generate(channel, plugin, channel.context.clone());
      serviceLocator.get('logger').info('Added stream for plugin', plugin.stream.getContext());
...
PluginVideo.prototype.onWatch = function(message) {
  ...
  this.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      var channel = new Channel(response['plugindata']['data']['result']['stream']['uid'], message['body']['id'], message['body']['channel_data'], plugin.context.clone());
      plugin.stream = Stream.generate(channel, plugin, channel.context.clone());
      serviceLocator.get('logger').info('Added stream for plugin', plugin.stream.getContext());
    }
    return Promise.resolve(response);
  ...
  plugin.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      return plugin.removeStream()
        .then(function() {
          var channel = new Channel(response['plugindata']['data']['result']['next']['uid'], message['body']['id'], message['body']['channel_data'], plugin.context.clone());
          plugin.stream = Stream.generate(channel, plugin, channel.context.clone());
          serviceLocator.get('logger').info('Added stream for plugin', plugin.stream.getContext());
