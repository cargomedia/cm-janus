...
PluginVideo.prototype.isValidMessage = function(message) {
  if (PluginVideo.super_.prototype.isValidMessage(message)) {
    return 'message' != message['janus'] || 'list' != message['body']['request'];
  } else {
    return false;
  }
};
...
PluginStreaming.prototype.onCreate = function(message) {
  ...
  this.session.connection.transactions.add(message['transaction'], function(response) {
    if (plugin._isSuccessResponse(response)) {
      ...
      return serviceLocator.get('cm-api-client').publish(plugin.stream.channelName, plugin.stream.id, Date.now() / 1000, plugin.session.data, plugin.stream.channelData)
        .then(function() {
