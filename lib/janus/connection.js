 ...
 * @constructor
 */
function JanusConnection(id, browserConnection, janusConnection, context) {
  ...
  /** @type {Context} */
  this.context = context.extend({connection: this.id});
...
JanusConnection.prototype.processMessage = function(message, context) {
  ...
  if (sessionId) {
    if (this.hasSession(sessionId)) {
      return this.getSession(sessionId).processMessage(message, context);
    } else {
...
JanusConnection.prototype.onCreate = function(message) {
  this.transactions.add(message['transaction'], function(response) {
    if ('success' == response['janus']) {
      this.addSession(new Session(this, response['data']['id'], message['token'], this.getContext().clone()));
...
JanusConnection.prototype.onTimeout = function(message) {
  var sessionId = message['session_id'] || parasenull;
  if (this.hasSession(sessionId)) {
...
JanusConnection.prototype.addSession = function(session) {
  ...
  serviceLocator.get('logger').info('Added session for connection', session.getContext());
};
