...
JanusConnection.prototype.processMessage = function(message) {
  ...
  if (pluginId) {
    serviceLocator.get('logger').warn('Plugin message without session', this.getContext().extend({request: message}));
...
JanusConnection.prototype.getContext = function() {
  ...
  if (this.browserConnection) {
    context.extend(this.browserConnection.getUrlObject().query);
  }
  return context;
};
