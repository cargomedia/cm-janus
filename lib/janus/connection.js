...
JanusConnection.prototype.processMessage = function(message) {
  ...
  if (pluginId) {
    serviceLocator.get('logger').error('Plugin message without session', this.getContext().extend({janus: {request: message}}));
...
JanusConnection.prototype.onTimeout = function(message) {
  var sessionId = message['session_id'] || null;
  if (this.hasSession(sessionId)) {
    return this._removeSession(sessionId).return(message);
  }
  return Promise.resolve(message);
};
...
JanusConnection.prototype.getContext = function() {
  var context = new Context({janus: {connectionId: this.id}});
