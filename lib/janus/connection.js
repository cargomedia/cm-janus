...
JanusConnection.prototype.processMessage = function(message) {
  ...
  if (pluginId) {
    serviceLocator.get('logger').warn('Plugin message without session', {request: message, context: this._context});
...
JanusConnection.prototype.onRemove = function() {
  ...
  return Promise.all(sessions.map(function(session) {
    ...
    .then(function() {
      serviceLocator.get('logger').info('Removed connection', {connection: this, context: self._context});
    });
};
...
JanusConnection.prototype.addSession = function(session) {
  ...
  serviceLocator.get('logger').info('Added session for connection', {session: session, connection: this, context: this._context});
};
