...
var ContextLogger = require('../ContextLogger');
...
function JanusConnection(id, browserConnection, janusConnection) {
  ...
  /** @type {ContextLogger} */
  this._logger = new ContextLogger();
...
JanusConnection.prototype.processMessage = function(message) {
  ...
  if (pluginId) {
    this.getLogger().warn('Plugin message without session: ' + message);
...
JanusConnection.prototype.onRemove = function() {
  ...
  return Promise.all(sessions.map(function(session) {
    ...
    .then(function() {
      self.getLogger().info('Removed ' + self);
    });
};
...
JanusConnection.prototype.addSession = function(session) {
  ...
  this.getLogger().info('Added ' + session + ' for ' + this);
};
 ...
 */
JanusConnection.prototype.getLogger = function() {
  return this._logger;
};
