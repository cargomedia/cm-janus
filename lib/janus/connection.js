...
var ContextLogger = require('../context-logger');
...
function JanusConnection(id, browserConnection, janusConnection, context) {
  ...
  /** @type {Object} */
  this._contextLogger = new ContextLogger(context);
}

JanusConnection.prototype.getContextLogger = function() {
...
JanusConnection.prototype.processMessage = function(message) {
  ...
  if (pluginId) {
    this.getContextLogger().warn('Plugin message without session', {request: message});
...
JanusConnection.prototype.onRemove = function() {
  ...
  return Promise.all(sessions.map(function(session) {
    ...
    .then(function() {
      self.getContextLogger().info('Removed connection', {connection: this});
    });
};
...
JanusConnection.prototype.addSession = function(session) {
  ...
  this.getContextLogger().info('Added session for connection', {session: session, connection: this});
};
