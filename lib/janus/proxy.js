...
JanusProxy.prototype.start = function() {
  ...
  return serviceLocator.get('streams').removeAll()
    .catch(function(error) {
      serviceLocator.get('logger').warn('Can\'t remove streams on startup', {error: error});
    })
    .then(function() {
      ...
      webSocketServer.on('connection', function(incomingConnection) {
        try {
          ...
          var context = proxy.extractContextFromConnection(fromClientConnection);
          proxy.establishConnection(fromClientConnection, toJanusConnection, context);

        } catch (error) {
          serviceLocator.get('logger').error('Unexpected JanusProxy runtime error', {error: error});
 ...
 */
JanusProxy.prototype.createConnection = function(fromClientConnection, toJanusConnection, context) {
  return new JanusConnection(uuid.v4(), fromClientConnection, toJanusConnection, context);
};
 ...
 * @returns {JanusConnection}
 */
JanusProxy.prototype.establishConnection = function(fromClientConnection, toJanusConnection, context) {
  var proxy = this;
  var connection = proxy.createConnection(fromClientConnection, toJanusConnection, context);
 ...
 * @returns {Context}
 */
JanusProxy.prototype.extractContextFromConnection = function(connection) {
  return new Context(connection.getUrlObject().query);
};
