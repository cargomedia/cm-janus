...
Session.prototype.getContextLogger = function() {
  return this.connection.getContextLogger();
};
...
Session.prototype.onAttach = function(message) {
  ...
  this.connection.transactions.add(message['transaction'], function(response) {
    if (response['janus'] === 'success') {
      ...
      self.getContextLogger().info('Added plugin for session', {plugin: plugin, session: self});
...
Session.prototype.onRemove = function() {
  ...
  return Promise.all(_.map(this.plugins, function(plugin, id) {
    ...
    self.getContextLogger().info('Removed session', {session: self});
  });
};
