 ...
 * @returns {Promise}
 */
PluginStreaming.prototype.onRtcup = function(message) {
  var self = this;
  return serviceLocator.get('cmApiClient').publish(this.stream.channelName, this.stream.id, Date.now())
    .then(function() {
      serviceLocator.get('logger').info('adding stream', self.stream);
      serviceLocator.get('streams').add(self.stream);
    })
    .catch(function(error) {
      serviceLocator.get('logger').info('Cannot publish: ', error);
