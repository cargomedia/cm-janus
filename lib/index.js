...
Application.prototype.start = function() {
  ...
  process.on('close', function() {
    ...
    Promise.all([jobManager.stop(), this._cleanupStreams()]).finally(function() {
      process.nextTick(process.exit(1));
    });
  }.bind(this));
};

Application.prototype._cleanupStreams = function() {
  var cmApiClient = serviceLocator.get('cm-api-client');
  var streamList = serviceLocator.get('streams').getList();

  return Promise.map(streamList, function(stream) {
    return cmApiClient.removeStream(stream.channelName, stream.id);
  });
};
