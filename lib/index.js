...
Application.prototype.start = function() {
  ...
  Promise.map(services, function(service) {
    ...
    serviceLocator.get('logger').error('Process failed. Exiting.', new Context({error: error}));
    process.emit('close');
  });

  process.on('unhandledRejection', function(reason) {
    serviceLocator.get('logger').error('Unexpected rejection error.', new Context({error: reason}));
  });

  process.on('uncaughtException', function(error) {
    serviceLocator.get('logger').error('Unexpected runtime error. Shutdown the process.', new Context({error: error}));
    process.emit('close');
  });
