...
var CmRestServer = function() {
  minilog.pipe(fs.createWriteStream('../' + config.get('App.logPath'), {flags: 'w', defaultEncoding: 'utf8'}));
  ...
  app.use(function(err, req, res, next) {
    ...
    minilog.error(errorMessage);
    res.status(err.code || 500).send({error: errorMessage});
  });

  process.on('uncaughtException', function(err) {
    minilog.error('Uncaught exception ' + err.message);
